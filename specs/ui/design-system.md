...
| 2026-06-13 | **Light theme restored + theme toggle re-added.** No longer dark-only: `:root` = light (warm off-white `--surface #faf9f7` + deep pink `--accent #D4537E`), `[data-theme="dark"]` = the charcoal+pink set from the 2026-06-11 redesign. Toggle via `components/theme-toggle.tsx`; theme applied pre-paint by an inline script in `layout.tsx`. **Font is Poppins** (`--font-sans`), not Plus Jakarta Sans. | Reconcile spec with shipped code |
