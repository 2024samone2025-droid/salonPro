...
## Working style
- Don't run `npm run dev` from the agent, and don't `npm run build` while the dev server runs (shared `.next`). Verify visually in the user's browser, not via automation.
