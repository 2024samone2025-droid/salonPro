...
function DayView({ appointments, currentTimeTop, overlapGroups, onAppointmentClick }: DayViewProps) {
          ...
          {appointments.map((apt) => {
            ...
            return (
              <div
                key={apt.id}
                className={cn(
                  "absolute rounded-lg border-l-[3px] p-1.5 cursor-pointer hover:opacity-90 transition-all overflow-hidden shadow-sm border",
...
export default function AppointmentsView() {
  ...
  const fetchAppointments = useCallback(async () => {
    ...
    try {
      const res = await authFetch(`/api/appointments?date=${selectedDate}&view=${viewMode}`)
